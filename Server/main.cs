...
string DiedSoundName = config["PlayerDied"]?["name"]?.Value<string>()?? "Null";
var DiedSound = new SoundPlayer(
    Path.Combine(
...
{
    if(DiedSoundName != "Null")
    {
        DiedSound.Play();
        Console.WriteLine($"[INFO]{DiedSoundName}已成功播放");
    }
};
